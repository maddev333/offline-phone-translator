...
function setTranslationOutput(text) {
  if (translationOutputEl) translationOutputEl.textContent = text || DEFAULT_TRANSLATION_OUTPUT;
}
...
function speakText(text) {
  ...
  stopSpeaking(); const utterance = new SpeechSynthesisUtterance(text); utterance.rate = 1; utterance.pitch = 1;
  ...
  utterance.onend = () => { isSpeakingResponse = false; setLocalConversationState("ok", "ready"); setLocalRecordingStatus("Text translation mode"); };
...
async function runLocalTranslation() {
  ...
  try {
    ...
    const text = await runLocalTranslationPrompt({ prompt: inputText, model: modelName, onToken: (chunk) => { if (chunk?.trim()) log("local output:", chunk); } });
